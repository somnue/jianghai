        ...
        <div class="container">
          <div class="box3Item wow fadeInUp">
            <div class="box3Bg-container">
              <ul class="box3Bg-top">
                <li @click="ProJumpLink(index)" v-for="(item,index)  in productList" :key='item.id'>
                  <p>{{item.product}}</p>
...
export default {
  ...
  methods: {
    ...
    getBusinessData () {
      ...
      this.$request
        .post('bus/selectAllBus', data, that.token)
        .then(res => {
          var Busdata = res.data.list
          for (let i in Busdata) {
            Busdata[i].businessImg =
              '../../static/images/businessIcon' + i + '.png'
          }
    ...
    BusJumpLink (index) {
      ...
      //   }
      // })
      // params传参 路径显示参数-页面刷新参数不丢失
      // that.$router.push({
      //   path: `/business/${that.businessList[index].businessId}/${that.businessList[index].business}/${that.businessList[index].businessEng}/${that.businessList[index].businessCon}`
      // })
      // params传参 路径不显示参数-页面刷新参数不丢失
      that.$router.push({
        name: 'business',
        params: {
          id: that.businessList[index].businessId,
    ...
    getproductData () {
      ...
      this.$request
        .post('product/selectAllProduct', data, that.token)
        .then(res => {
          var Prodata = res.data.list
          for (let i in Prodata) {
            Prodata[i].productImg =
              '../../static/images/productIcon' + i + '.png'
          }
    ...
    ProJumpLink (index) {
      ...
      // })
      that.$router.push({
        path: `/product/${that.productList[index].productId}${that.productList[index].product}`
      })
    },
    onMouseOver (index) {
      var that = this
    ...
    backgroundImg () {
      ...
      var data = {}
      that.$request
        .post('file/homePage/query', data, that.token)
        .then(res => {
          console.log(res.data.list)
          that.bannerImgUrl =
            'http://47.105.89.34:7005/officialWeb-service/file/download?filePath=' +
            res.data.list[2].picturePath
          that.mediaImgUrl =
            'http://47.105.89.34:7005/officialWeb-service/file/download?filePath=' +
            res.data.list[1].picturePath
          that.productsImgUrl =
            'http://47.105.89.34:7005/officialWeb-service/file/download?filePath=' +
            res.data.list[0].picturePath
        })
        .catch(err => {
          console.log(err)
        })
...
.business ul li {
  ...
  display: block;
  background: #fff;
  box-shadow: 0 5px 9px 0 rgba(0, 83, 204, 0.3);
  border-radius: 10px;
  width: 180px;
  height: 250px;
  margin-right: 10px;
...
.business ul li .businessIconHover img {
  animation: pic 0.5s linear 0s 1 forwards;
  -webkit-animation: pic 0.5s linear 0s 1 forwards;
}
@keyframes pic {
  from {
...
.business ul li .businessIconHover p {
  ...
  margin-bottom: 12px;
  line-height: 20px;
  animation: title 0.5s linear 0s 1 forwards;
  -webkit-animation: title 0.5s linear 0s 1 forwards;
}
@keyframes title {
  from {
...
.business ul li .businessIconHover div {
  ...
  margin: 0 auto;
  text-align: left;
  animation: word 0.5s linear 0s 1 forwards;
  -webkit-animation: word 0.5s linear 0s 1 forwards;
}
...
.box3Bg ul li:hover {
  ...
  animation: top 0.5s linear 0s 1 forwards;
  -webkit-animation: top 0.5s linear 0s 1 forwards;
}
